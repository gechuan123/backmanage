    ...
    form.on('submit(add)', function(data){
		var index = layer.confirm('よろしいですか？', {icon: 3, title:'荻原建設',btn: ['確定', 'キャンセル']}, function(){
			$.ajax({
				...
				type: 'POST',
				data: {id:data.field.id,pid:data.field.pid,title:data.field.title,name:data.field.name,sort:data.field.sort,status:data.field.status,remark:data.field.remark},
				dataType: 'JSON',
